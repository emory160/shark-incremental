...
LANGUAGES.KO = {
    ...
    text: {
        ...
        'all-research': {
            ...
            'm9': [`방사능 오버플로우 제거`,`${toTextStyle('방사선 '+icon('radioactive'),'core')} 생산의 소프트캡이 사라집니다.`],
            'm10': [`자동 핵-원자로 MK2`,`자동-${toTextStyle('핵','core')} 원자로가 세번째 줄의 ${toTextStyle('핵','core')} 원자로를 구매합니다.`],
            'm11': [`자동-채광 업그레이드 MK2`,`자동-${toTextStyle('상어','shark')} 채광 업그레이드가 나머지 ${toTextStyle('상어','shark')} 채광 업그레이드도 자동으로 구매합니다.`],
        ...
        'evolution-tree-ctn': [
            ...
            ["강해진 열 1", x=>`첫 번째 열에 있는 이 위의 모든 업그레이드가 <b>${formatPercent(x-1)}</b> 더 강해집니다.`, x=>`이 트리 위에 있는 첫 번째 줄의 모든 트리와 강화된 트리가 <b>${formatPercent(x-1)}</b> 만큼 강해집니다.`],
            ["강해진 열 2", x=>`두 번째 열에 있는 이 위의 모든 업그레이드가 <b>${formatPercent(x-1)}</b> 더 강해집니다.`, x=>`이 트리 위에 있는 두 번째 줄의 모든 트리와 강화된 트리가 <b>${formatPercent(x-1)}</b> 만큼 강해집니다.`],
            ...
            ["진정한 열 2", x=>`진화 트리의 두 번째 열을 제한 없이 구매할 수 있습니다.`, x=>`강화된 트리의 두 번째 열을 제한 없이 구매할 수 있습니다.`],
            ["진정한 열 3", x=>`진화 트리의 세 번째 열을 제한 없이 구매할 수 있습니다.`, x=>`강화된 트리의 세 번째 열을 제한 없이 구매할 수 있습니다.`],
            ["진정한 열 4", x=>`진화 트리의 네 번째 열을 제한 없이 구매할 수 있습니다.`, x=>`강화된 트리의 네 번째 열을 제한 없이 구매할 수 있습니다.`],
        ],
        ...
        'solar-system-list': {
            ...
            'pluto':   ["명왕성",`명왕성은 태양계에서 가장 큰 왜소행성으로 알려진 해왕성 바깥 천체이자 태양을 도는 천체 중<br>8개의 행성과 에리스에 이어 10 번째로 무거운 천체입니다. 암석과 얼음으로 이루어져 있으며<br>질량은 달의 약 1/6배, 부피는 약 1/3배 정도입니다. 명왕성의 궤도는 이심률이 크며 황도면에 대한<br>경사각이 크기에 태양과의 거리가 큰 편차를 보입니다. 가장 가까울 때는 29.7 AU(천문단위)이고,<br>가장 멀 때는 49.3 AU입니다.`, `더 많은 ${toTextStyle('잔재물','black-hole')} 업그레이드를 해금하며 ${toTextStyle('별자리','star')} 재화가 더 이상 초기화 되지 않습니다.`],
        },
        ...
        'experiment-boosts': [
            ...
            x=>`${toTextStyle('궤도','traject')} 획득량이 ${x} 만큼 증가합니다.`,
        ],
